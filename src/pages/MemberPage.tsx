...
export default function MemberPage() {
  ...
  // Check if inactive member
  const isInactive = !member.isActive && !member.isAdmin;
  ...
  const handleApplyLoan = () => {
    if (!loanAmount || Number(loanAmount) <= 0 || Number(loanAmount) > memberMaxLoan) return;
      ...
      <div className="max-w-lg mx-auto px-4 py-3">
          ...
          <div className="space-y-4">
            ...
            {isInactive && (
              <div className="bg-gradient-to-r from-orange-500/20 to-amber-500/20 border border-orange-500/30 rounded-2xl p-4">
                <p className="text-orange-300 font-bold text-sm">⚠️ {t('inactiveMemberBanner')}</p>
                <p className="text-orange-200/70 text-xs mt-1">{t('inactiveFundEarning')}</p>
              </div>
            )}
            ...
            {/* Download CSV Button */}
            <button onClick={handleDownloadCSV} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500/20 to-teal-500/20 hover:from-cyan-500/30 hover:to-teal-500/30 border border-cyan-500/30 text-cyan-300 rounded-2xl p-3 font-semibold text-sm transition-all active:scale-[0.98]">
              <Download className="w-5 h-5" />
              {t('exportCSV')} - {member.name}
            </button>
        ...
        {activeTab === 'loans' && (
          <div className="space-y-4">
            ...
            {isInactive && canLoan && (
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 text-orange-300 text-sm">
                <AlertTriangle className="w-4 h-4 inline mr-1" />
                {t('inactiveLoanLimit', { max: formatCurrency(memberMaxLoan) })}
              </div>
            )}
          ...
          <div className="bg-slate-800 rounded-t-3xl p-6 w-full max-w-lg border border-white/10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="text-xs text-gray-400 space-y-1">
                ...
                <p>📌 {t('lateFeeInfo')}</p>
                {!member.isActive ? (
                  <p>📌 {t('inactiveLoanLimit', { max: formatCurrency(memberMaxLoan) })}</p>
                ) : (
                  <p>📌 {t('loanLimitInfo')}</p>
                )}
              </div>
